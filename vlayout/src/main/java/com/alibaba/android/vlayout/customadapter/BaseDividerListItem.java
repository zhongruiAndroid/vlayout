...
//    private int mOrientation;
    ...
    public void drawHorizontalFirstLine(Canvas c, RecyclerView parent, RecyclerView.LayoutParams params) {
        ...
        final int top =bottom - dividerHelper.getFirstDividerHeight();
        mDivider.setBounds(left, top, right, bottom);
        mDivider.draw(c);
    }
    public void drawHorizontalLineForReverse(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //画第一条线
            if(i == 0&&dividerHelper.isShowFirstDivider()){
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if(itemPosition==parent.getAdapter().getItemCount()-1) {
            outRect.set(0, 0, 0, 0);
        }else if(dividerHelper.isMyAdapter(adapter)&&itemPosition==parent.getAdapter().getItemCount()-2) {//最后第二个item设置偏移//垂直列表
            if (orientation == VERTICAL_LIST) {
                int offset=0;
                if(dividerHelper.isShowLastDivider()){
                    offset=dividerHelper.getLastDividerWidth()>getDividerHeight()?getDividerHeight():dividerHelper.getLastDividerWidth();
                }
                if(reverseLayout){
            ...
            if (orientation == VERTICAL_LIST) {
                //列表是否反向
                if(reverseLayout){
                    outRect.set(0, getDividerHeight(), 0, dividerHelper.getFirstDividerHeightOffset(itemPosition));
                }else{
                    outRect.set(0,dividerHelper.getFirstDividerHeightOffset(itemPosition),  0, getDividerHeight());
                }
            } else {//水平列表
                //列表是否反向
                if(reverseLayout){
